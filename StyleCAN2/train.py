...
def train(args, loader, generator, discriminator, g_optim, d_optim, g_ema, device):
    ...
    for idx in pbar:
        ...
        if args.use_CAN:
            fake_pred, style_logits_fake = discriminator(fake_img, return_logits=True)
        else:
            fake_pred = discriminator(fake_img, return_logits=True)

        if args.use_CAN:
            # compute CAN loss
            style_logits_fake_probs = F.log_softmax(style_logits_fake)
            CAN_loss = -args.CAN_weight * (style_logits_fake_probs / style_logits_fake_probs.size(1)).mean()            
        ...
        if args.use_CAN:
            CAN_loss_val = loss_reduced["CAN_loss"].mean().item()
            classf_loss_val = loss_reduced["classif_loss"].mean().item()

        if get_rank() == 0:
            description = f"d: {d_loss_val:.4f}; g: {g_loss_val:.4f}; r1: {r1_val:.4f}; "\
                    f"path: {path_loss_val:.4f}; mean path: {mean_path_length_avg:.4f}; "\
                    f"augment: {ada_aug_p:.4f}; "

            if args.use_CAN:
                description += f"CAN: {CAN_loss_val:.4f}; classification: {classf_loss_val:.4f}"            
            ...
            if wandb and args.wandb:
                ...
                if args.use_CAN:
                    log_dict['Classification Loss'] = classf_loss_val
                    log_dict['CAN loss'] = CAN_loss_val                
                wandb.log(log_dict)
