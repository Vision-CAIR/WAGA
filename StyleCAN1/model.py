...
class Discriminator(nn.Module):
    def __init__(self, fused=True, from_rgb_activate=False, n_discs=[]):
        super().__init__()
        ...
        if n_discs != []:
            self.linears = nn.ModuleList([self.linear] + [EqualLinear(512, k) for k in n_discs])
    ...
    def forward(self, input, step=0, alpha=-1):
        ...
        if hasattr(self, 'linears'):
            out = [l(out) for l in self.linears]
        else:
            out = self.linear(out)
